        ...
        for orbit_index in range(1,number_of_orbits_in_sh+1 , 1):
            ...
            for satellite_index in range(1 , number_of_satellites_per_orbit+1 , 1):
                ...
                if orbit_index != number_of_orbits_in_sh:
                    ...
                    # Mostafa added this part
                    # Instead of a simple mapping, we find the physically closest satellite in the first orbit.
                    # We only need to check this at a single time step (e.g., t=1) as the closest neighbor relationship is stable.
                    first_orbit = sh.orbits[0]
                    for target_satellite in first_orbit.satellites:
                        dist = distance_two_satellites(cur_satellite, target_satellite, 1) # Check distance at t=1
                        if dist < min_distance:
                            min_distance = dist
                            right_satellite = target_satellite
                            right_satellite_id = target_satellite.id # We found a new closest satellite
