...
def visualization_constellation_without_ISL(constellation_information):
...
def visualization_constellation_with_ISL(constellation_information):
    ...
    for shell in constellation_information:
        ...
        #shorest_path = [1124, 538, 1147, 603, 1169, 647, 1213, 691, 1238, 1280, 1324, 1368, 1412, 998, 1434]
        # shorest_path = [1124, 538,1147]
        sat_id=1
        for j in range(len(satellites)):
            satellites[j]["satellite"].compute("1949-10-01 00:00:00")
            ...
            """
            # only starlink use
            # 分别画出从Mexico City到Barcelona大圆上的短路和长路
            # Mexico City sat_id = 472/1124
            # Barcelona sat_id = 1434

            content_string += (
                    "var redSphere = viewer.entities.add({name : '" + str(sat_id) + "', position: Cesium.Cartesian3.fromDegrees(" \
                    + str(math.degrees(satellites[j]["satellite"].sublong)) + ", " \
                    + str(math.degrees(satellites[j]["satellite"].sublat)) + ", "
                    + str(satellites[j]["altitude"] * 1000) + "), " \
                    + "ellipsoid : {radii : new Cesium.Cartesian3(30000.0, 30000.0, 30000.0), " \
                    + "material : Cesium.Color.BLACK.withAlpha(0.1),}, " \
                    + "label : new Cesium.LabelGraphics({ text : '" + (
                            str(sat_id)
                    ) \
                    + "', font : '10pt sans-serif', fillColor: Cesium.Color.BLACK,outlineColor: " \
            ...
            # print("sat_id = ",sat_id , " , sat_id_true = " , satellites[j]["sat_id"])
        ...
        color = ['BLUE', 'DEEPSKYBLUE', 'LAWNGREEN', 'MEDIUMSEAGREEN']
        ...
        red_nodes = []
        shorest_path = [1124, 538, 1147, 603, 1169, 647, 1213, 691, 1257, 1280, 1324, 1368, 1412, 998, 1434]
        for i in range(len(shorest_path)):
            print(shorest_path[i]-1)
            red_nodes.append(satellites[shorest_path[i]-1])
        for index in range(len(red_nodes)-1):

            sat1 = red_nodes[index]
            sat2 = red_nodes[index+1]
            print(sat1["sat_id"] , "-->" , sat2["sat_id"])
            content_string += (
                    "viewer.entities.add({name : '', polyline: { positions: Cesium.Cartesian3.fromDegreesArrayHeights([" \
                    + str(math.degrees(sat1["satellite"].sublong)) + "," \
                    + str(math.degrees(sat1["satellite"].sublat)) + "," \
                    + str(sat1["altitude"] * 1000) + "," \
                    + str(math.degrees(sat2["satellite"].sublong)) + "," \
                    + str(math.degrees(sat2["satellite"].sublat)) + "," \
                    + str(sat2["altitude"] * 1000) + "]), " \
                    + "width: 6, arcType: Cesium.ArcType.NONE, " \
                    + "material: new Cesium.PolylineOutlineMaterialProperty({ " \
                    + "color: Cesium.Color.RED"
                    + ".withAlpha(4), outlineWidth: 4, outlineColor: Cesium.Color.RED})}});")

        yellow_nodes = []
        longest_path = [1124,1102,1058,1014,991,969,925,924,902,858,1487,1422,813,1378,769,1334,725,659,1267,1289,
                        1266,1244,1265,1243,
                        310,1220,244,179,1154,135,1110,69,1087,25,1065,1021,1434]
        for i in range(len(longest_path)):
            print(longest_path[i] - 1)
            yellow_nodes.append(satellites[longest_path[i] - 1])
        for index in range(len(yellow_nodes) - 1):
            sat1 = yellow_nodes[index]
            sat2 = yellow_nodes[index + 1]
            print(sat1["sat_id"], "-->", sat2["sat_id"])
            content_string += (
                    "viewer.entities.add({name : '', polyline: { positions: Cesium.Cartesian3.fromDegreesArrayHeights([" \
                    + str(math.degrees(sat1["satellite"].sublong)) + "," \
                    ...
                    + ".withAlpha(4), outlineWidth: 4, outlineColor: Cesium.Color.YELLOW})}});")
...
def constellation_visualization(constellation_name , xml_file_path ,output_file_path,
                                head_html_file , tail_html_file ,ISL = False):
    ...
    if ISL:
        ...
        writer_html = open(output_file_path + constellation_name + "_with_ISL_shortest_longest.html", 'w')
        ...
        # 只可视化星座中的卫星，不可视化ISL
        visualization_content = visualization_constellation_without_ISL(constellation_information)
        writer_html = open(output_file_path + constellation_name + "_without_ISL_shortest_longest.html", 'w')
        with open(head_html_file, 'r') as fi:
            writer_html.write(fi.read())
        writer_html.write(visualization_content)
        with open(tail_html_file, 'r') as fb:
...
if __name__ == '__main__':
    ...
    constellation_visualization(constellation_name , xml_file_path,output_file_path,
                                head_html_file , tail_html_file , True)
